...
try {
    if(((functionTestChkLst6.value == "☑" && functionTestChkconformLst6.value == "適合") || functionTestChkLst6.value == "☒")){
        $this.select("合格");
    }else if($this.value == "合格"){
        $this.select("");
    }
    if(! isEmpty($this.value)){
        if(isEmpty(functionTestingDate3.value)){
            functionTestingDate3.value = formatDate(new Date(), "dd-MMM-yyyy");
        }
        if(isEmpty(functionTestingResponsible3.value)){
            var currentUser = sdk.getBean("CurrentUserProperties");
            var fullName = currentUser.getFullName();
            functionTestingResponsible3.value = fullName;
        }
    }
} catch (e) {
    alert(e);
